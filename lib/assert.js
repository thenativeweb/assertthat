...
is = {
  ...
  throwingMsg: function(wantToBe)
  {
      var expected = (typeof(wantToBe) == "undefined") ? '': wantToBe;
      return function(actual)
      {
          var test = is.throwing();
          var result = test(actual); 
          if(!result.success)
            {
                return {
                    message: 'nothin was thrown',
                    success: false
                };
            }
            else{
                ...
                else
                {
                    //Test Reg-Expr. too
                    return{
                       message: util.format('Expected %s to be the same as %s.', result.error, expected),
                       success: false
                    };
                }
            }
